...
- **Pinned story.** From 1024px the "Follow one sick day" section pins: the heading, the three steps, and the device stage hold still while the page scrolls, the scroll position moves the story from step 1 to 3 (70vh of scrolling each), and only after step 3 does the section let go. Scroll position is the single source of truth; the player and the clickable step titles only scroll the page. The text column is the wider of the two so the descriptions run in long lines, the steps are spaced well apart, the heading sits near the top of the pinned block, and the device panel is capped at 37rem tall. The current step expands with CSS grid rows (`0fr` to `1fr`), never a measured height: animating to `height: "auto"` makes Motion restore the scroll position, which cancels a smooth scroll in flight. Below 1024px the steps are a plain list, each with its own screenshot.
