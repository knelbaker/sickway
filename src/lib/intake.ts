...
const TIME_OF_DAY: [RegExp, number][] = [
  [/\bmorning\b/, 8],
  [/\bnoon\b|\bmidday\b|\blunch/, 12],
  [/\bafternoon\b/, 14],
  [/\bevening\b/, 19],
  [/\bnight\b|\btonight\b/, 22],
];
...
export function suggestOnsetIso(phrase: string | null, fixtureClock: string): string | null {
  ...
  const hoursAgo = /\b(\d+|an?|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)\s+hours?\s+ago\b/.exec(text);
  if (hoursAgo) {
  ...
  const daysAgo = /\b(\d+|an?|one|two|three|four|five|six|seven)\s+days?\s+ago\b/.exec(text);
  if (daysAgo) daysBack = count(daysAgo[1]);
  else if (/\blast night\b|\byesterday\b/.test(text)) daysBack = 1;
  else if (/\btoday\b|\bthis (morning|afternoon|evening)\b|\btonight\b|\bwoke up\b/.test(text)) daysBack = 0;
